...
- **RESOLVIDO (2026-07-06) via backfill DJEN**: `preencher_classe_via_djen` popula `Process.classe_codigo` da movimentação DJEN **mais recente** (100% dos movs trazem `codigoClasse`) — independe do Datajud. Usa a mais recente (não arbitrária) pra não pegar a classe de um mov de expedição de precatório (1265) em vez da real (Cumprimento 12078). Rodar por tribunal: `--tribunal TJSP`. TJSP subiu de ~39% → ~84% de classe preenchida.
- **Regra de sinal ligada no TJSP (2026-07-06)**: `PRECATORIO_SINAL_TRIBUNAIS` e `PAGAMENTO_SINAL_TRIBUNAIS` agora incluem TJSP (antes só TJAL/TJMA). Cumprimento contra Fazenda (`CLASSES_FAZENDA_PUBLICA`) + expedição nos movs → PRECATORIO score 1.0 (o LR v6 trava e-SAJ em ≤0,582); guard F24 veta os já pagos. Reclassificar: `reclassificar_tjsp_precatorio --apply`. Materializou ~3,7k PRECATORIO + ~22,6k PRE_PRECATORIO no TJSP.
- ⚠️ **Bug de blanking corrigido (2026-07-06, `bcf809e`)**: `enrichers/drainer.py::normalize_dados` apagava `classe_codigo` quando o e-SAJ dava a classe só com **nome** (sem código TPU) → a cada re-enriquecimento o F1 zerava e o lead revertia a NAO_LEAD. Afetava 87% dos TJSP enriquecidos. Agora só sobrescreve o código se vier código; senão preserva.
- **Regra de sinal NEGATIVA universal — `F30_extinto_neg_ANTI` (2026-07-08)**: desfecho terminal negativo lido do TEXTO das movs (extinção **sem resolução do mérito** / improcedência / indeferimento da inicial / **prescrição** decretada / nego provimento) que seja a última palavra — **sem expedição posterior** (`extneg_max_dt >= exped_max_dt`) → rebaixa QUALQUER lead (PRECATORIO/PRE/DC) a **NAO_LEAD** com score 0,10, em **todos os tribunais** (diferente do F24, que é só TJMA/TJSP e exige expedição prévia). Override fora dos pesos LR (como F19/F24; não entra em `HARDCODED_WEIGHTS`, logo não afeta `_validate_pesos`). Motivo: processo extinto/improcedente NÃO é lead comprável, por mais que a classe diga "Cumprimento de Sentença" — descobrimos com o TJAL 0726440-50.2026 (extinto art. 924, I CPC) sendo servido como "lead quente". A mesma leitura de desfecho está no dossiê determinístico (`jurimetria_dossie._desfecho`) e no contexto da narrativa IA. ⚠️ o número **F30** foi escolhido pra não colidir com o F25 planejado no v7. Re-classify cirúrgico dos leads quentes: re-rodar `classificar_e_persistir` nos PRECATORIO/PRE_PRECATORIO.
