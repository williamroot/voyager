...
def publish(payload: dict, redis_client=None) -> str:
    """Publica um evento de enriquecimento no stream. Retorna o ID gerado
    pelo Redis (timestamp-seq). Workers chamam isso em vez de gravar no DB.

    `MAXLEN ~ STREAM_MAXLEN` é defense-in-depth: se o drainer travar, o
    stream rotaciona entries antigas em vez de crescer sem fim. O
    `approximate=True` (~) deixa o Redis trimmar em chunks (mais rápido).
    Drainer também faz XDEL após XACK no caminho normal.
    """
    r = redis_client or get_redis()
    msg_id = r.xadd(
        STREAM_KEY, to_redis_fields(payload),
        maxlen=STREAM_MAXLEN, approximate=True,
    )
    return msg_id.decode() if isinstance(msg_id, bytes) else msg_id


def ensure_consumer_group(redis_client=None) -> None:
    """Cria o consumer group se ainda não existir. Idempotente.

    `id='0'` faz o group entregar tudo desde o início do stream — útil pra
    re-deploy do drainer sem perder eventos. MKSTREAM cria o stream caso
    ainda não exista (1ª vez em produção).
    """
    r = redis_client or get_redis()
    try:
